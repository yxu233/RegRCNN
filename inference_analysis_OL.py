        ...
        with torch.no_grad():
            ...
            if cf.dim == 2:
                ...
                if len(np.unique(seg_im)) == 1:
                    ...
                    label_arr = np.copy(batch['seg'])
                    new_labels = np.zeros(np.shape(batch['seg']))
                    for box_id, box_row in enumerate(results_2to3D['boxes'][0]):
                        
                        #print(box_row['box_score'])
                        if box_row['box_score'] > cf.merge_3D_iou:
                            box_arr = np.zeros(np.shape(batch['seg']))
