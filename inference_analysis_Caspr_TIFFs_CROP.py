...
if __name__=="__main__":
    class Args():
        def __init__(self):
            ...
            self.exp_dir = '/media/user/FantomHD/710_invivo_imaging/Caspr_tdT_homozygous/Caspr_training/Caspr_training_RegRCNN/2) 3D training/'
            
            
            self.exp_dir = '/media/user/FantomHD/710_invivo_imaging/Caspr_tdT_homozygous/Caspr_training_CROP/Caspr_training_CROP_RegRCNN/3) 3D_training_1class/'
    ...
    #list_folder = ['/media/user/FantomHD/710_invivo_imaging/Caspr_tdT_homozygous/Caspr_RegRCNN_testing/Caspr_RegRCNN_testing_output_PYTORCH/']
    
    list_folder = ['/media/user/FantomHD/710_invivo_imaging/Caspr_tdT_homozygous/Caspr_RegRCNN_test_timeseries/Caspr_RegRCNN_test_timeseries_output_PYTORCH/']
    ...
    for input_path in list_folder:
        ...
        examples = [dict(input=i,seg=i.replace('_input_im.tif','_segmentation_LABELLED.tif'), ilastik=i.replace('.tif','_single_Object Predictions_.tiff')) for i in images]
        ...
        for i in range(len(examples)):
            ...
            seg_labels_im = tiff.imread(seg_name)
            seg_labels_im = np.moveaxis(seg_labels_im, 0, -1)
            ...
            width = im_size[1];  height = im_size[2]; depth_im = im_size[3];
            ...
            cc = measure.regionprops(seg_labels_im)
            
            segmentation = np.zeros([width, height, depth_im])
            
            for paranode_num, paranode in enumerate(cc):
                
                from functional.matlab_crop_function import *
                
                c = paranode['coords']
                center = paranode['centroid']
                z = np.round(center[2]).astype(int)
                x = np.round(center[0]).astype(int)
                y = np.round(center[1]).astype(int)
                
                crop_size = np.asarray(np.round(input_size/2), dtype=np.int64)
                z_size = depth
                
                
                
                """ NEED TO CROP WITH PADDING??? """
                
                crop_seg, box_x_min, box_x_max, box_y_min, box_y_max, box_z_min, box_z_max = crop_around_centroid(seg_labels_im, y, x, z, 
                                                                                                             crop_size, z_size, 
                                                                                                             height, width, depth_im)
                crop_seg = np.copy(crop_seg)
                
                ### set all other values to zero
                p_id = seg_labels_im[x, y, z]
                
                crop_seg[crop_seg != p_id] = 0
                
                crop_seg[crop_seg > 0] = 255  ### then set current value to 255 so always the same
                    
                
                
                
                crop_input = input_im[:, box_x_min:box_x_max, box_y_min:box_y_max,  box_z_min:box_z_max]
                quad_intensity = np.copy(crop_input)  ### otherwise will overwrite exisiting array
                   
                
                
                ### REPLACE THE 3rd channel with the seg_labels_im crop
                quad_intensity[2, ...] = crop_seg
                ...
                if cf.dim == 2:
                     ...
                     if len(np.unique(seg_im)) == 1:
                         ...
                         if len(np.unique(label_arr)) == 1:  ### due to box score being super low!!!
                             continue   ### no objects found (only 0 - background)      
                ...
                cleaned_seg = np.asarray(class_labels, dtype=np.uint8)
                cleaned_seg = cleaned_seg[0, 0, :,  :, :]
             
             
             
                """ Make it a colored array by giving this current seg a number so we know what's paired """
             
                cleaned_seg[cleaned_seg > 0] = paranode_num
                ...
                segmentation[box_x_min:box_x_max, box_y_min:box_y_max,  box_z_min:box_z_max] = cleaned_seg       
                
                
                ### this is adding, it is bad
                #segmentation[box_x_min:box_x_max, box_y_min:box_y_max,  box_z_min:box_z_max] = cleaned_seg + segmentation[box_x_min:box_x_max, box_y_min:box_y_max,  box_z_min:box_z_max]
             
        
                crop_save = np.copy(quad_intensity[0])  ### crop_input has the raw data, while quad_intensity has the seed mask
                
                # add segmentation as extra channel               
                crop_save = np.concatenate((crop_save, np.expand_dims(cleaned_seg, axis=0)))
                
                
                
                crop_save = np.moveaxis(crop_save, -1, 0)
                crop_save = np.expand_dims(crop_save, axis=0)
                
                crop_save = np.asarray(crop_save, dtype=np.uint16)
                
                
                
                # tiff.imwrite(sav_dir + filename + '_' + str(int(paranode_num)) +'_crop_save.tif', crop_save,
                #                      imagej=True, #resolution=(1/XY_res, 1/XY_res),
                #                      metadata={'spacing':1, 'unit': 'um', 'axes': 'TZCYX'})  
                        
                
                ### save 2D view
                # ma = plot_max(crop_save[0, :, 0, :, :], ax=0, plot=0)  ### plot figure 1 max projection
                # tiff.imwrite(sav_dir + filename + '_MAX_' + str(int(paranode_num)) +'_INPUT_crop_save.tif', np.asarray(ma, dtype=np.uint8))    

                # ma = plot_max(crop_save[0, :, 2, :, :], ax=0, plot=0)  ### plot figure 1 max projection
                # tiff.imwrite(sav_dir + filename + '_MAX_' + str(int(paranode_num)) +'_MASK_crop_save.tif', np.asarray(ma, dtype=np.uint8))
                
                # ma = plot_max(crop_save[0, :, 3, :, :], ax=0, plot=0)  ### plot figure 1 max projection
                # tiff.imwrite(sav_dir + filename + '_MAX_' + str(int(paranode_num)) +'_SEG_crop_save.tif', np.asarray(ma, dtype=np.uint8))
