    ...
    for input_path in list_folder:
        ...
        for i in range(len(examples)):
           ...
           with torch.set_grad_enabled(False):  # saves GPU RAM            
                ...
                for z in range(0, depth_im + Lpatch_depth, round(Lpatch_depth)):
                    if z + Lpatch_depth > depth_im:  continue

                    for x in range(0, width + Lpatch_size, round(Lpatch_size)):
                          if x + Lpatch_size > width:  continue

                          for y in range(0, height + Lpatch_size, round(Lpatch_size)):
                               if y + Lpatch_size > height: continue
                ...
                #for id_c in range(93, len(all_xyz)):
                        ...
                        for x in range(0, width + patch_size, round(step_xy)):
                              ...
                              for y in range(0, height + patch_size, round(step_xy)):
                                   ...
                                   if len(np.unique(seg_im)) == 1 or len(box_score) == 0:
                                        ...
                                        # continue if no objects (i.e. the edge subtraction cleaned up all objects in FOV)
                                        
                                        new_labels = np.asarray(new_labels, dtype=int)
                                        if len(regionprops(new_labels[0][0])) == 0:
                                            continue
                    ...
                    tiff.imwrite(sav_dir + filename + '_' + str(int(id_c)) +'_split_seg.tif', split_seg)
    
    
                    segmentation = np.asarray(segmentation, np.uint16)
                    tiff.imwrite(sav_dir + filename + '_' + str(int(id_c)) +'_whole_seg.tif', segmentation)
